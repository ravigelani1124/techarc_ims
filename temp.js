...
const organizationSchema = new mongoose.Schema({
  ...
  org_name_en: {type: String,required: [true, "Organization name is required"],},
  org_name_fr: {type: String,required: [false, "Organization name is required"],},
  org_email: {type: String,unique: true, match: [/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/,"Invalid email format",],trim: true,},
  org_phone: {type: String,unique: true, minlength: [10, "Phone number must be 10 digits long"],maxlength: [10, "Phone number must be 10 digits long"],},
