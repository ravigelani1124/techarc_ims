...
const Login = () => {
  ...
  const handleSignUpClick = (e) => {
    ...
    if (
      ...
      console.log('validateSignUp', emailSignUp, passwordSignUp, nameSignUp, privateKey)
      setErrorMessage('All Fields Required*')
      setAlertVisible(true) // Show the alert
    } else {
      setAlertVisible(false) // Hide the alert if previously shown
  ...
  const handleSubmitSignUp = useCallback(() => {
    fetchDataSignUp()
  }, [emailSignUp, passwordSignUp, nameSignUp, privateKey])
  ...
  const fetchDataSignUp = async () => {
    console.log('fetchDataSignUp')
    setIsLoading(true)
    try {
      const response = await axios.post('http://localhost:3000/api/auth/supersignup', {
        email: emailSignUp,
  ...
  const handleLoginClick = (e) => {
    ...
    if (emailLogin.length === 0 || passwordLogin.length === 0) {
      console.log('validateLogin', emailLogin, passwordLogin)
      setErrorMessage('All Fields Required*')
      setAlertVisible(true) // Show the alert
    } else {
      setAlertVisible(false) // Hide the alert if previously shown
      console.log('Login API 1')
      handleSubmitLogin()
    }
  }
  ...
  const fetchDataLogin = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await axios.post('http://localhost:3000/api/auth/superlogin', {
        email: emailLogin,
              ...
              <CCard className="p-4">
                <CCardBody>
                  <CForm>
                    ...
                    <CInputGroup className="mb-3"></CInputGroup>
                    <CRow>
                      <CCol xs={6}>
